...
#   age_at_i <- age_ring[i]
#   if(age_at_i=='adult'){
#     first_cap <- which(blackcap[i,]==1)[1]
#     age[i, first_cap:ncol(blackcap)] <- age[i, first_cap:ncol(blackcap)]+2
#     
#   }
#   else if (age_at_i=='juvenile'){
#     first_cap <- which(blackcap[i,]==1)[1]
...
#       age[i, first_cap:ncol(blackcap)] <- age[i, first_cap:ncol(blackcap)]+1
#     }
#     else{
#       if(first_cap+7<=77){
#         age[i, first_cap:(first_cap+6)]<-age[i, first_cap:(first_cap+6)]+1
#         age[i, (first_cap+7):ncol(blackcap)]<-age[i, (first_cap+7):ncol(blackcap)]+2
#       }
#     }
#   }
# }
...
ex_m <- expected_m_array(R ,phi.open_juve.blackcap, phi.open_adul.blackcap, 
                         p.open.blackcap, T)
