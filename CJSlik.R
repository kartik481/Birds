...
CJSlik <- function(theta, x, f, l, age, n, T) {
  ...
  # age - matrix representing age of birds (1 for juveniles, 2 for adults)
  ...
  for (i in 1:n){
    ...
    if (f[i] != l[i]){
      for (t in f[i]:(l[i]-1)){
        if (age[i, t] == 1) {
          # If the individual is a juvenile at time t
          phi_t <- exp(theta[t]) / (1 + exp(theta[t]))
        } else {
          # If the individual is an adult at time t
          phi_t <- exp(theta[t] + theta[T+1]) / (1 + exp(theta[t] + theta[T+1]))
        }
        lik[i] <- lik[i] + log(phi_t) + x[i, t+1] * log(p) + (1 - x[i, t+1]) * log(1 - p)
      }
    }
    ...
    # I.e. we do not have any histories such that f = l = T.
